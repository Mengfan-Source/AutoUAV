...
//#include <map_os0_room2.h>
//#include <tower_test.h>
//#include <map_single_room_new1.h>
//#include <index.h>
#include <index_itrlab05.h>
...
void Occupy_map::init(ros::NodeHandle& nh)
{
 ...
 //   occupancy_buffer_ = occupancy_buffer_os0_room2;
}
...
void Occupy_map::inflate_point_cloud(void)
{
    ...
    //                 // 设置膨胀后的点被占据
    //                // this->setOccupancy(p3d_inf, 1);//将膨胀后的点云地图存入occupancy_buffer_（三维地图变二维数组）
    //             }
    //     // ROS_INFO("d%",sizeof(occupancy_buffer_));
    // }
    //将膨胀之后的点云数据三维地图转化为二维数组，之后保存在txt文件中，下次直接进行文件读取出数据。
    // std::ofstream fout;
    // fout.open("/home/xmf/aaaa.txt",std::ios::app);
    // if(fout.is_open())
    // {
    //     for(vector<int>::iterator m=occupancy_buffer_.begin(),end=occupancy_buffer_.end(); m!=end; ++m)
    //     {
    //         fout<<(*m);
    //         fout<<(",");
    //     }
    //    // occupancy_buffer_.size();
    // }
    // else{
    //     ROS_INFO("the txt cant be opened");
    // }
    // fout.close();
   ...
   //occupancy_buffer_ = occupancy_buffer;
...
int Occupy_map::getOccupancy(Eigen::Vector3d pos) 
{
    ...
    posToIndex(pos, id);
     /***************************************************self****************************************************************/
     if (Occupy_map::search(index_itrlab05,id(0) * grid_size_(1) * grid_size_(2) + id(1) * grid_size_(2) + id(2)) > -1)
     {
        return 1;
     }
     else
...
int Occupy_map::search(std::vector<int>& nums, int target) 
    {
// 定义左右区间
        int left=0;
