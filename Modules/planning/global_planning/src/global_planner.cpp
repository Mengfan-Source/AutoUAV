...
void Global_Planner::drone_state_cb(const prometheus_msgs::DroneStateConstPtr& msg)
{
    ...
    Drone_odom.twist.twist.linear.z = _DroneState.velocity[2];//self:当飞机状态数据来了之后，更新程序中的里程计数据（其实就是更新了当前的位姿数据）
}
...
{
    ROS_INFO("run into Gpointcloud_cb ");
    sensor_ready = true;
    Gpointcloud_sub.shutdown();//不再订阅这个话题？？？自己写
    static unsigned int cloud_update_time = 0;
    cloud_update_time ++;
    if(cloud_update_time>=0xffffff)
    {
        cloud_update_time  = 3;
    }
    else if(cloud_update_time <=1)
    {
            ...
            Astar_ptr->Occupy_map_ptr->map_update_gpcl(msg);//self：更新全局点云
            Astar_ptr->Occupy_map_ptr->inflate_point_cloud(); //对点云进行膨胀
            ROS_INFO("map_groundtruth[%d]",map_groundtruth);
            ROS_INFO("Occupied map processing has beenning finished! ");
    ...
    else if (cloud_update_time<0xffffffff &&cloud_update_time>1)
    {
       // ROS_INFO("Occupied map processing has beenning finished!");
    }
    else
    {
         ROS_INFO("there is something wrong in Occupied map processing! ");
         ROS_WARN("there is something wrong in Occupied map processing! ");
    }   
}
