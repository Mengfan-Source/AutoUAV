...
void laser_cb(const tf2_msgs::TFMessage::ConstPtr &msg)
{
    ...
    //有的时候/tf这个消息的发布者不止一个
    //可改成ＴＦ监听
    if (msg->transforms[0].header.frame_id == "camera_init" && msg->transforms[0].child_frame_id == "body" && input_source == 1)  
//yuan:msg->transforms[0].header.frame_id == "map" && msg->transforms[0].child_frame_id == "base_link" && input_source == 1
    {
        ...
        pos_drone_laser[2] = laser.transform.translation.z + pos_offset[2]+0.3; 
         ...
         /******************************************自己写self******************************************************
         double my_pi = 3.1415926535897932384626433832795029;
         Eigen::AngleAxisd rotation_vector(my_pi,Eigen::Vector3d(0,0,1));//self：定义旋转矩阵，绕z轴旋转180度
         Eigen::Matrix3d rotation_matrix = rotation_vector.toRotationMatrix();//self：将旋转矩向量转变为旋转矩阵
         Eigen::Quaterniond q_180z =  Eigen::Quaterniond(rotation_matrix);//self 将旋转矩阵转变为四元数
         q_180z.normalize();
         q_laser_enu = q_180z * q_laser_enu;//self 进行旋转变换
        pos_drone_laser[0] = -pos_drone_laser[0];
        pos_drone_laser[1] = -pos_drone_laser[1];
        ******************************************自己写self******************************************************/
...
int main(int argc, char **argv)
{
    ...
    //ROS_INFO("offset[%f][%f][%f][%f]",pos_offset[0],pos_offset[1],pos_offset[2],yaw_offset);
...
void send_to_fcu()
{
    ...
    //vision.header.frame_id = "camera_init";
